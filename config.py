...
def get_parser():
    ...
    parser.add_argument("--loader_mode", type=str, default="workers4", choices=["workers4", "persistent", "gpu"], help="workers4 (default, unchanged): DataLoader(num_workers=4), rebuilt every task. persistent: adds persistent_workers+pin_memory so workers survive the 50 epochs within a task. gpu: the whole subset lives on the GPU as tensors and batching is manual indexing -- no DataLoader at all. NOTE gpu changes RNG consumption (no sampler draws), so trajectories will not match bitwise.")
    parser.add_argument("--make_plots", action="store_true", help="Render the per-task coherence-matrix figure and the end-of-run trajectory figures. Off by default: the only consumer is wandb.log, which is a no-op under WANDB_MODE=disabled, and the tick-label layout costs ~42 s/cell on headless runs. Does not affect any logged number.")
