...
def get_parser():
    ...
    parser.add_argument(
        "--adaptive_reg", action="store_true", help="Enable 1/tau regularization"
    )
    parser.add_argument(
        "--adaptive_scope",
        type=str,
        ...
        help="Scope for adaptive regularization",
    )
    parser.add_argument(
        "--adaptive_type",
        type=str,
        default="l2",
        choices=["l2", "spectral", "wass", "parseval"],
    ...
    parser.add_argument(
        "--underparameterized",
        action="store_true",
        help="Use a single hidden layer MLP (fc1 -> act -> output), skipping fc2",
    )
    parser.add_argument(
        "--feature_diagnostics",
        action="store_true",
        help="Compute task-end NFM/AGOP diagnostics and save plots per task",
    )
    parser.add_argument(
        "--feature_diag_every",
        type=int,
        default=1,
        help="Run NFM/AGOP diagnostics every k tasks",
    )
    parser.add_argument(
        "--feature_diag_samples",
        type=int,
        default=512,
        help="Maximum number of samples used to estimate AGOP at each task end",
    )
    parser.add_argument(
        "--feature_diag_batch_size",
        type=int,
        default=64,
        help="Batch size used during AGOP estimation",
    )
    parser.add_argument(
        "--feature_diag_layers",
        type=str,
    ...
    parser.add_argument(
        "--feature_diag_dir",
        type=str,
        default="plots/feature_diagnostics",
        help="Directory where task-end NFM/AGOP diagnostics are saved",
    )
    return parser
