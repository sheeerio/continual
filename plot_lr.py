...
Example:
  python plot_lr.py \
    ...
    --match-mode exact \
    --runs1 "crelu_f_1e-2_ply_svar10ll_250" "crelu_f_1e-2_250" \
    --runs2 "l2_f_1e-3_ply_ss10_250" "l2_f_1e-3_250" \
    --runs3 "wass_f_1e-3_ply_sqm10rrl_250" "wass_f_1e-3_250"  \
    --labels1 "100" "250" "500" \
    --labels2 "100" "250" "500" \
    --labels3 "100" "250" "500" \
    --lr-key hessian_rank --smooth 0.0 \
    --show-points \
    --max-points 2000 \
    --xmax 100000 \
    --out plots/hessian_3x1.pdf \
    --colors1 "#d53e4f" "#f46d43" "#3288bd" \
    --colors2 "#d53e4f" "#f46d43" "#3288bd" \
    --colors3 "#d53e4f" "#f46d43" "#3288bd" \
    --title1 "CReLU (lr=0.01)" \
    --title2 "L2 ($\\lambda=0.001$)" \
    --title3 "Wasserstein ($\\lambda=0.001$)" \
    --ymin1 0 --ymin2 0 --ymin3 0
...
Three subplots; each subplot overlays three chosen runs.
- X-axis is _STEP (native step), not task index.
- Each run is picked explicitly by name (exact or substring match).
- Optional EMA smoothing per run (applied to LR).
- Per-run plotting only (no cross-seed aggregation/align-by-task).
- Optional group filter; if provided, only runs from that W&B group are considered.
- Y-axis is LOG scale (learning rate).
- X-axes are aligned across all panels/runs (shared global xmax unless --xmax is given).

Example:
  python plot_lrs_3x1.py \
    --entity sheerio --project workshop_MNIST --group ka \
    --match-mode exact \
    --runs1 "crelu_f_1e-2_..." "crelu_f_1e-2_..." "crelu_f_1e-2_..." \
    --runs2 "l2_f_1e-3_..." "l2_f_1e-3_..." "l2_f_1e-3_..." \
    --runs3 "wass_f_1e-3_..." "wass_f_1e-3_..." "wass_f_1e-3_..." \
    --labels1 "100" "250" "500" \
    --labels2 "100" "250" "500" \
    --labels3 "100" "250" "500" \
    --lr-key lr --smooth 0.0 \
    --show-points \
    --max-points 2000 \
    --xmax 24000 \
    --out plots/lr_3x1.pdf \
    --colors1 "#d53e4f" "#f46d43" "#3288bd" \
    --colors2 "#d53e4f" "#f46d43" "#3288bd" \
    --colors3 "#d53e4f" "#f46d43" "#3288bd" \
    --title1 "CReLU" \
    --title2 "L2 ($\\lambda=0.001$)" \
    --title3 "Wasserstein ($\\lambda=0.001$)" \
    --ymin1 3e-4 --ymax1 6e-3 \
    --ymin2 3e-4 --ymax2 6e-3 \
    --ymin3 3e-4 --ymax3 6e-3
"""

import argparse, os, hashlib, math
import numpy as np
import pandas as pd
...
def parse_args():
    ...
    # metrics & plotting
    p.add_argument("--lr-key", default="hessian_rank")
...
def main():
    ...
    for pi, (ax, tokens, entries) in enumerate(zip(axes, panels, collected), start=1):
        ...
        ax.set_xlabel("Step")
        if pi == 1: ax.set_ylabel("Hessian rank")
