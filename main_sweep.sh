    ...
    for lr in "${LRS[@]}"; do
      for coef in "${COEFS[@]}"; do
        for seed in "${SEEDS[@]}"; do
          ...
          CMD="python3 implicit_regularization.py --seed ${seed} --optimizer adam --activation ${ACT} --runs 20 --epochs 100 --ns 1.0 --lr ${lr} --dataset MNIST --lr_schedule constant --track_coherence --coherence_window 100 ${REG_FLAGS} --name=${NAME} --exp_name=${EXP_NAME}"
          COMMANDS+=("${CMD}")
        done
      done
    done
