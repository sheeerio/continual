...
Example:
  python plot_new.py \
    ...
    --group sweeps \
    --match-mode exact \
    --name  "sweeps_bn_l2_lr1e-5_wd1e-2" \
    --name2 "sweeps_wass_lr1e-5_lam1e-3" \
    --label1 "bn+l2,lr=1e-5" --label2 "wass,lr=1e-5" --color2 tab:orange \
    --acc-key task_acc \
    --rank-key average_update_norm \
    --rank-stat last \
    --smooth-seeds 0.1 \
    --band std \
    --min-seeds 2 \
    --out plots/update_norm_false.pdf
"""
...
def main():
    ...
    axR.set_ylabel("Update norm (dashed)", fontsize=20)
    title = f""
    axL.set_title(title)
    ...
    # axL.legend(hL + hR, lL + lR, loc="best", fontsize=15)
    handles, labels = (hL + hR), (lL + lR)

    # anchor at the right edge of the x-axis (slightly inset)
    axR.relim()
    axR.autoscale_view()
    axR.margins(y=0.05)  # small breathing room

    # Legend: use axes coords so it's stable regardless of data scale
    leg = axR.legend(
        handles, labels,
        loc="center left",
        bbox_to_anchor=(0.6, 0.25),   # (x,y) in axes coords
        bbox_transform=axR.transAxes,  # <-- key change
        fontsize=15,
        frameon=True,
    )


    # optional: keep it above the plotting artists
    # leg.set_zorder(10)
