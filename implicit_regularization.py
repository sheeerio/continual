...
parser.add_argument("--wass_lambda", type=float, default=0.0,
     help="lambda coefficient for Wasserstein regularization")
...
if args.reg == "wass":
    for name, p0 in init_params.items():
        init_params[name] = torch.sort(p0.view(-1))[0].to(device)
...
for i in range(10, 10 + args.runs):
    ...
    for _ in range(EPOCHS):
        for inputs, labels in train_loader:
            ...
            if args.reg == "l2_init":
                ...
                for name, p in model.named_parameters():
                    if p.requires_grad:
                        sorted_cur = torch.sort(p.view(-1))[0]
                        sorted_init = init_params[name]
                        reg_term += (sorted_cur - sorted_init).pow(2).sum()
                reg_term *= args.wass_lambda
            ...
            if total_updates % args.log_interval == 0:
                ...
                h_in = inputs
                if args.activation == "relu":
                    if args.model == "BatchNormMLP":
                        h = F.relu(model.bn1(model.fc1(h_in)))
                        h = F.relu(model.bn2(model.fc2(h)))
                        h = F.relu(model.bn3(model.fc3(h)))
                    elif args.model == "LayerNormMLP":
                        h = F.relu(model.ln1(model.fc1(h_in)))
                        h = F.relu(model.ln2(model.fc2(h)))
                        h = F.relu(model.ln3(model.fc3(h)))
                    else:
    ...
    if args.activation == "relu":
        if args.model == "BatchNormMLP":
            h = F.relu(model.bn1(model.fc1(inputs)))
            h = F.relu(model.bn2(model.fc2(h)))
            h = F.relu(model.bn3(model.fc3(h)))
        elif args.model == "LayerNormMLP":
            h = F.relu(model.ln1(model.fc1(inputs)))
            h = F.relu(model.ln2(model.fc2(h)))
            h = F.relu(model.ln3(model.fc3(h)))
        else:
