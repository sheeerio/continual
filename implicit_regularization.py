...
if __name__ == "__main__":
    ...
    for i in range(10, 11, 2):
        ...
        print(f"Randomizing {i+1}0% of the dataset")
        train_dataset_c = randomize_targets(train_dataset, i/10)
        train_set, val_set = torch.utils.data.random_split(train_dataset_c, [45000, 5000])
        print("Train set size", len(train_set))
        print("Validation set size", len(val_set))
        print("Test set size", len(test_dataset))
        train_loader = data.DataLoader(train_set, batch_size=128, shuffle=True, num_workers=1)
        val_loader = data.DataLoader(val_set, batch_size=128, shuffle=False, num_workers=1)
        test_loader = data.DataLoader(test_dataset, batch_size=128, shuffle=False, num_workers=1)
