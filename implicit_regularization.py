...
def make_diag_run_name(cfg):
    ...
    cleaned = []
    for part in parts:
        token = str(part).replace(os.sep, "-").replace(" ", "_")
        cleaned.append(token or "empty")
...
feature_diag_init_weights = feature_diagnostics.capture_initial_linear_weights(model)
feature_diag_run_name = make_diag_run_name(config)
...
act_layer_states = {
    layer: misc.EMAState(alphas=(0.01, 0.05, 0.5)) for layer in layer_map
}
...
if config.optimizer == "adam":
    ...
    optimizer = torch.optim.SGD(layer_groups, lr=config.lr, weight_decay=wd)
...
for task in range(config.runs):
    ...
    if config.reset_model:
        ...
        if config.optimizer == "adam":
            ...
            optimizer = optim.SGD(layer_groups, lr=config.lr, weight_decay=wd)
    ...
    for epoch in range(config.epochs):
        for x, y in loader:
            ...
            if getattr(config, "adaptive_reg", False):
                ...
                global_factor = None
                if reg_scope == "global":
                    global_lam = optimizers.estimate_hessian_topk(
                        model, base, params, k=1, iters=1
                    )[0]
                    global_norm_lam = optimizers.get_norm_sharpness(
                        optimizer, global_lam, config
                    )
                    global_eff_lr = effective_lr

                    sharp_state, global_scalars = misc.update_stat(
                        global_norm_lam, sharp_state, global_eff_lr
                    )
                    ...
                    if total_updates % config.log_interval == 0:
                        wandb.log(
                            {"global/inv_tau_penalty": global_factor}, commit=False
                        )
                ...
                for layer, l_params in layer_map.items():
                    ...
                    state, scalars = misc.update_stat(
                        norm_lam, layer_states[layer], l_eff_lr
                    )
                    act_state, act_scalars = misc.update_stat(
                        lam, act_layer_states[layer], l_eff_lr
                    )
                    ...
                    if reg_scope == "global":
                        ...
                        rig_list = [
                            float(scalars["rig_ema_1"]),
                            float(scalars["rig_ema_2"]),
                            float(scalars["rig_ema_3"]),
                        ]
                        ...
                        if total_updates % config.log_interval == 0:
                            wandb.log(
                                {
                                    f"{layer}/rig_mix": rig_mix,
                                    f"{layer}/rig_ema_1": rig_list[0],
                                    f"{layer}/rig_ema_2": rig_list[1],
                                    f"{layer}/rig_ema_3": rig_list[2],
                                },
                                commit=False,
                            )
                    ...
                    if reg_type == "spectral":
                        for p in l_params:
                            if p.ndim >= 2 and p.requires_grad:
                                ...
                                layer_reg_val += (
                                    sigma.pow(config.spectral_k) - 1.0
                                ).pow(2)
                    elif reg_type == "wass":
                        for p in l_params:
                            if p.requires_grad:
                    ...
                    elif reg_type == "parseval":
                        for p in l_params:
                            if p.ndim >= 2 and p.requires_grad:
                                W = p.view(p.shape[0], -1)
                                I = torch.eye(W.shape[1], device=p.device)
                                layer_reg_val += torch.norm(W.t() @ W - I, p="fro") ** 2
                    else:
                        layer_reg_val = sum(
                            p.pow(2).sum() for p in l_params if p.requires_grad
                        )
                    reg += adaptive_factor * layer_reg_val

                    if total_updates % config.log_interval == 0:
                        wandb.log(
                            {f"{layer}/reg_factor": adaptive_factor}, commit=False
                        )
            ...
            if total_updates % config.log_interval == 0:
                ...
                for layer, params in layer_map.items():
                    if layer in step_stats:
                        ...
                        norm_lam = optimizers.get_norm_sharpness(optimizer, lam, config)
                        eff_lr = layer_eff_lrs.get(
                            layer, optimizer.param_groups[0]["lr"]
                        )
                        state, scalars = misc.update_stat(
                            norm_lam, layer_states[layer], eff_lr
                        )
                        act_state, act_scalars = misc.update_stat(
                            lam, act_layer_states[layer], eff_lr
                        )
                    ...
                    gi = torch.autograd.grad(
                        loss, params, retain_graph=True, allow_unused=False
                    )
                    g_layer_sq = float(
                        torch.cat([g.contiguous().view(-1) for g in gi])
                        .pow(2)
                        .sum()
                        .item()
                    )
                    ...
                    s_svar_sigma2_l10 = (
                        sigma2_l + 20.0 * g_layer_sq * scalars["lam_var"]
                    )
                    s_sqm_sigma2_l10 = sigma2_l + 20.0 * g_layer_sq * scalars["sq_mean"]
                    s_rcv_sigma2_l1 = (
                        sigma2_l + 10.0 * g_layer_sq * act_scalars["lam_cv"]
                    )
                    s_rs_sigma2_l1 = sigma2_l + 10.0 * g_layer_sq * lam
                    s_rvar_sigma2_l1 = (
                        sigma2_l + 10.0 * g_layer_sq * act_scalars["lam_var"]
                    )
                    s_rsqm_sigma2_l1 = (
                        sigma2_l + 75.0 * g_layer_sq * act_scalars["sq_mean"]
                    )
                    s_rcv_sigma2_l10 = (
                        sigma2_l + 20.0 * g_layer_sq * act_scalars["lam_cv"]
                    )
                    s_rs_sigma2_l10 = sigma2_l + 20.0 * g_layer_sq * lam
                    s_rvar_sigma2_l10 = (
                        sigma2_l + 20.0 * g_layer_sq * act_scalars["lam_var"]
                    )
                    s_rsqm_sigma2_l10 = (
                        sigma2_l + 20.0 * g_layer_sq * act_scalars["sq_mean"]
                    )

                    r = (sigma2_l / max(1, inputs.size(0))) / (
                        g_layer_sq + 1e-12
                    ) + 1e-12
                    ...
                    alpha_crit_scv_layer1 = (B * g_layer_sq) / max(
                        s_scv_sigma2_l1, 1e-12
                    )
                    alpha_crit_ss_layer1 = (B * g_layer_sq) / max(s_ss_sigma2_l1, 1e-12)
                    alpha_crit_svar_layer1 = (B * g_layer_sq) / max(
                        s_svar_sigma2_l1, 1e-12
                    )
                    alpha_crit_sqm_layer1 = (B * g_layer_sq) / max(
                        s_sqm_sigma2_l1, 1e-12
                    )
                    alpha_crit_scv_layer10 = (B * g_layer_sq) / max(
                        s_scv_sigma2_l10, 1e-12
                    )
                    alpha_crit_ss_layer10 = (B * g_layer_sq) / max(
                        s_ss_sigma2_l10, 1e-12
                    )
                    alpha_crit_svar_layer10 = (B * g_layer_sq) / max(
                        s_svar_sigma2_l10, 1e-12
                    )
                    alpha_crit_sqm_layer10 = (B * g_layer_sq) / max(
                        s_sqm_sigma2_l10, 1e-12
                    )
                    alpha_crit_rcv_layer1 = (B * g_layer_sq) / max(
                        s_rcv_sigma2_l1, 1e-12
                    )
                    alpha_crit_rs_layer1 = (B * g_layer_sq) / max(s_rs_sigma2_l1, 1e-12)
                    alpha_crit_rvar_layer1 = (B * g_layer_sq) / max(
                        s_rvar_sigma2_l1, 1e-12
                    )
                    alpha_crit_rsqm_layer1 = (B * g_layer_sq) / max(
                        s_rsqm_sigma2_l1, 1e-12
                    )
                    alpha_crit_rcv_layer10 = (B * g_layer_sq) / max(
                        s_rcv_sigma2_l10, 1e-12
                    )
                    alpha_crit_rs_layer10 = (B * g_layer_sq) / max(
                        s_rs_sigma2_l10, 1e-12
                    )
                    alpha_crit_rvar_layer10 = (B * g_layer_sq) / max(
                        s_rvar_sigma2_l10, 1e-12
                    )
                    alpha_crit_rsqm_layer10 = (B * g_layer_sq) / max(
                        s_rsqm_sigma2_l10, 1e-12
                    )
                    ...
                    union_eff_gt_acrit_step_scv1 = max(
                        union_eff_gt_acrit_step_scv1, eff_gt_acrit_scv1
                    )
                    union_eff_gt_acrit_step_ss1 = max(
                        union_eff_gt_acrit_step_ss1, eff_gt_acrit_ss1
                    )
                    union_eff_gt_acrit_step_svar1 = max(
                        union_eff_gt_acrit_step_svar1, eff_gt_acrit_svar1
                    )
                    union_eff_gt_acrit_step_sqm1 = max(
                        union_eff_gt_acrit_step_sqm1, eff_gt_acrit_sqm1
                    )
                    union_eff_gt_acrit_step_scv10 = max(
                        union_eff_gt_acrit_step_scv10, eff_gt_acrit_scv10
                    )
                    union_eff_gt_acrit_step_ss10 = max(
                        union_eff_gt_acrit_step_ss10, eff_gt_acrit_ss10
                    )
                    union_eff_gt_acrit_step_svar10 = max(
                        union_eff_gt_acrit_step_svar10, eff_gt_acrit_svar10
                    )
                    union_eff_gt_acrit_step_sqm10 = max(
                        union_eff_gt_acrit_step_sqm10, eff_gt_acrit_sqm10
                    )
                    union_eff_gt_acrit_step_rcv1 = max(
                        union_eff_gt_acrit_step_rcv1, eff_gt_acrit_rcv1
                    )
                    union_eff_gt_acrit_step_rs1 = max(
                        union_eff_gt_acrit_step_rs1, eff_gt_acrit_rs1
                    )
                    union_eff_gt_acrit_step_rvar1 = max(
                        union_eff_gt_acrit_step_rvar1, eff_gt_acrit_rvar1
                    )
                    union_eff_gt_acrit_step_rsqm1 = max(
                        union_eff_gt_acrit_step_rsqm1, eff_gt_acrit_rsqm1
                    )
                    union_eff_gt_acrit_step_rcv10 = max(
                        union_eff_gt_acrit_step_rcv10, eff_gt_acrit_rcv10
                    )
                    union_eff_gt_acrit_step_rs10 = max(
                        union_eff_gt_acrit_step_rs10, eff_gt_acrit_rs10
                    )
                    union_eff_gt_acrit_step_rvar10 = max(
                        union_eff_gt_acrit_step_rvar10, eff_gt_acrit_rvar10
                    )
                    union_eff_gt_acrit_step_rsqm10 = max(
                        union_eff_gt_acrit_step_rsqm10, eff_gt_acrit_rsqm10
                    )
                    ...
                    wandb.log(
                        {
                            ...
                            f"{layer}/alpha_crit_t": alpha_crit_t,
                            ...
                            f"{layer}/alpha_crit_svar10": float(
                                alpha_crit_svar_layer10
                            ),
                            ...
                            f"{layer}/alpha_crit_rvar10": float(
                                alpha_crit_rvar_layer10
                            ),
                            f"{layer}/alpha_crit_rsqm10": float(
                                alpha_crit_rsqm_layer10
                            ),
                            "reg": reg,
                        }
                    )
                    ...
                    if config.lr_schedule == "pl_lyapunov":
                        ...
                        if config.param == "sqm10":
                            lr_star = pl_scheduler.step(
                                layer,
                                eff_lr,
                                alpha_crit_sqm_layer10,
                                total_updates,
                                total_steps,
                            )
                        elif config.param == "t":
                            lr_star = pl_scheduler.step(
                                layer, eff_lr, alpha_crit_t, total_updates, total_steps
                            )
                        elif config.param == "sqm1":
                            lr_star = pl_scheduler.step(
                                layer,
                                eff_lr,
                                alpha_crit_sqm_layer1,
                                total_updates,
                                total_steps,
                            )
                ...
                norm_sharpness = optimizers.get_norm_sharpness(
                    optimizer, sharpness, config
                )
                ...
                sharp_state, sharp_log = misc.update_stat(
                    norm_sharpness, sharp_state, effective_lr
                )
                r_sharp_state, r_sharp_log = misc.update_stat(
                    sharpness, r_sharp_state, effective_lr
                )
            ...
            if config.sam:
                ...
                if total_updates % config.log_interval == 0:
                    ...
                    T_t, sigma2_hat = snr_tracker.update(
                        grad_flat.detach(), eta_eff, int(batch_B)
                    )
                    if T_t is not None:
                        snr_T_series.append((total_updates, float(T_t)))

                    sigma2_hat_mb = optimizers.grad_variance_within_batch(
                        model, criterion_nored, inputs, labels
                    )
                    k_rs_sigma2_hat_mb1 = (
                        sigma2_hat_mb + 10.0 * true_grad_norm_sq * sharpness
                    )
                    k_ss_sigma2_hat_mb1 = (
                        sigma2_hat_mb + 10.0 * true_grad_norm_sq * norm_sharpness
                    )
                    k_rs_sigma2_hat_mb10 = (
                        sigma2_hat_mb + 20.0 * true_grad_norm_sq * sharpness
                    )
                    k_ss_sigma2_hat_mb10 = (
                        sigma2_hat_mb + 20.0 * true_grad_norm_sq * norm_sharpness
                    )
                    s_scv_sigma2_hat_mb10 = (
                        sigma2_hat_mb + 20.0 * true_grad_norm_sq * sharp_log["lam_cv"]
                    )
                    s_svar_sigma2_hat_mb10 = (
                        sigma2_hat_mb + 20.0 * true_grad_norm_sq * sharp_log["lam_var"]
                    )
                    s_rcv_sigma2_hat_mb10 = (
                        sigma2_hat_mb + 20.0 * true_grad_norm_sq * r_sharp_log["lam_cv"]
                    )
                    s_rvar_sigma2_hat_mb10 = (
                        sigma2_hat_mb
                        + 20.0 * true_grad_norm_sq * r_sharp_log["lam_var"]
                    )  # decent contender w/ a smaller coeff
                    s_scv_sigma2_hat_mb1 = (
                        sigma2_hat_mb + 10.0 * true_grad_norm_sq * sharp_log["lam_cv"]
                    )
                    s_svar_sigma2_hat_mb1 = (
                        sigma2_hat_mb + 10.0 * true_grad_norm_sq * sharp_log["lam_var"]
                    )
                    s_rcv_sigma2_hat_mb1 = (
                        sigma2_hat_mb + 10.0 * true_grad_norm_sq * r_sharp_log["lam_cv"]
                    )
                    s_rvar_sigma2_hat_mb1 = (
                        sigma2_hat_mb
                        + 10.0 * true_grad_norm_sq * r_sharp_log["lam_var"]
                    )
                    s_ssqm_sigma2_hat_mb10 = (
                        sigma2_hat_mb + 20.0 * true_grad_norm_sq * sharp_log["sq_mean"]
                    )
                    s_rsqm_sigma2_hat_mb10 = (
                        sigma2_hat_mb
                        + 20.0 * true_grad_norm_sq * r_sharp_log["sq_mean"]
                    )
                    s_ssqm_sigma2_hat_mb1 = (
                        sigma2_hat_mb + 75.0 * true_grad_norm_sq * sharp_log["sq_mean"]
                    )
                    s_rsqm_sigma2_hat_mb1 = (
                        sigma2_hat_mb
                        + 10.0 * true_grad_norm_sq * r_sharp_log["sq_mean"]
                    )

                    r = (sigma2_hat_mb / max(1, batch_B)) / true_grad_norm_sq + 1e-12
                    eta_crit = min(0.8 / r, 2 / (norm_sharpness * (1 + r)))
                    T_t_mb = (
                        eta_eff * (sigma2_hat_mb / max(1, batch_B)) / true_grad_norm_sq
                    )
                    ...
                    K_rs_t_mb1 = (
                        eta_eff
                        * (k_rs_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    K_ss_t_mb1 = (
                        eta_eff
                        * (k_ss_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    K_rs_t_mb10 = (
                        eta_eff
                        * (k_rs_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    K_ss_t_mb10 = (
                        eta_eff
                        * (k_ss_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )

                    S_scv_t_mb10 = (
                        eta_eff
                        * (s_scv_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_svar_t_mb10 = (
                        eta_eff
                        * (s_svar_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_rcv_t_mb10 = (
                        eta_eff
                        * (s_rcv_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_rvar_t_mb10 = (
                        eta_eff
                        * (s_rvar_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )

                    S_scv_t_mb1 = (
                        eta_eff
                        * (s_scv_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_svar_t_mb1 = (
                        eta_eff
                        * (s_svar_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_rcv_t_mb1 = (
                        eta_eff
                        * (s_rcv_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_rvar_t_mb1 = (
                        eta_eff
                        * (s_rvar_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )

                    S_ssqm_t_mb1 = (
                        eta_eff
                        * (s_ssqm_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_rsqm_t_mb1 = (
                        eta_eff
                        * (s_rsqm_sigma2_hat_mb1 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_ssqm_t_mb10 = (
                        eta_eff
                        * (s_ssqm_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    S_rsqm_t_mb10 = (
                        eta_eff
                        * (s_rsqm_sigma2_hat_mb10 / max(1, batch_B))
                        / true_grad_norm_sq
                    )
                    ...
                    pred, pred_real, meanT, thresh, conf = snr_predictor.update(
                        T_t_mb, T_t
                    )
                    (
                        K_rs_pred1,
                        ...
                        K_rs_thresh1,
                        K_rs_conf1,
                    ) = snr_predictor.update(K_rs_t_mb1, T_t)
                    (
                        K_ss_pred1,
                        K_ss_pred_real1,
                        K_ss_meanT1,
                        K_ss_thresh1,
                        K_ss_conf1,
                    ) = snr_predictor.update(K_ss_t_mb1, T_t)
                    (
                        K_rs_pred10,
                        K_rs_pred_real10,
                        K_rs_meanT10,
                        K_rs_thresh10,
                        K_rs_conf10,
                    ) = snr_predictor.update(K_rs_t_mb10, T_t)
                    (
                        K_ss_pred10,
                        K_ss_pred_real10,
                        K_ss_meanT10,
                        K_ss_thresh10,
                        K_ss_conf10,
                    ) = snr_predictor.update(K_ss_t_mb10, T_t)
                    (
                        S_scv_pred10,
                        S_scv_pred_real10,
                        S_scv_meanT10,
                        S_scv_thresh10,
                        S_scv_conf10,
                    ) = snr_predictor.update(S_scv_t_mb10, T_t)
                    (
                        S_svar_pred10,
                        S_svar_pred_real10,
                        S_svar_meanT10,
                        S_svar_thresh10,
                        S_svar_conf10,
                    ) = snr_predictor.update(S_svar_t_mb10, T_t)
                    (
                        S_rcv_pred10,
                        S_rcv_pred_real10,
                        S_rcv_meanT10,
                        S_rcv_thresh10,
                        S_rcv_conf10,
                    ) = snr_predictor.update(S_rcv_t_mb10, T_t)
                    (
                        S_rvar_pred10,
                        S_rvar_pred_real10,
                        S_rvar_meanT10,
                        S_rvar_thresh10,
                        S_rvar_conf10,
                    ) = snr_predictor.update(S_rvar_t_mb10, T_t)
                    (
                        S_scv_pred1,
                        S_scv_pred_real1,
                        S_scv_meanT1,
                        S_scv_thresh1,
                        S_scv_conf1,
                    ) = snr_predictor.update(S_scv_t_mb1, T_t)
                    (
                        S_svar_pred1,
                        S_svar_pred_real1,
                        S_svar_meanT1,
                        S_svar_thresh1,
                        S_svar_conf1,
                    ) = snr_predictor.update(S_svar_t_mb1, T_t)
                    (
                        S_rcv_pred1,
                        S_rcv_pred_real1,
                        S_rcv_meanT1,
                        S_rcv_thresh1,
                        S_rcv_conf1,
                    ) = snr_predictor.update(S_rcv_t_mb1, T_t)
                    (
                        S_rvar_pred1,
                        S_rvar_pred_real1,
                        S_rvar_meanT1,
                        S_rvar_thresh1,
                        S_rvar_conf1,
                    ) = snr_predictor.update(S_rvar_t_mb1, T_t)
                    (
                        S_ssqm_pred1,
                        S_ssqm_pred_real1,
                        S_ssqm_meanT1,
                        S_ssqm_thresh1,
                        S_ssqm_conf1,
                    ) = snr_predictor.update(S_ssqm_t_mb1, T_t)
                    (
                        S_rsqm_pred1,
                        S_rsqm_pred_real1,
                        S_rsqm_meanT1,
                        S_rsqm_thresh1,
                        S_rsqm_conf1,
                    ) = snr_predictor.update(S_rsqm_t_mb1, T_t)
                    (
                        S_ssqm_pred10,
                        S_ssqm_pred_real10,
                        S_ssqm_meanT10,
                        S_ssqm_thresh10,
                        S_ssqm_conf10,
                    ) = snr_predictor.update(S_ssqm_t_mb10, T_t)
                    (
                        S_rsqm_pred10,
                        S_rsqm_pred_real10,
                        S_rsqm_meanT10,
                        S_rsqm_thresh10,
                        S_rsqm_conf10,
                    ) = snr_predictor.update(S_rsqm_t_mb10, T_t)

                    alpha_crit_t = (batch_B * true_grad_norm_sq) / max(
                        sigma2_hat_mb, 1e-12
                    )
                    alpha_crit_k_rs1 = (batch_B * true_grad_norm_sq) / max(
                        k_rs_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_k_ss1 = (batch_B * true_grad_norm_sq) / max(
                        k_ss_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_k_rs10 = (batch_B * true_grad_norm_sq) / max(
                        k_rs_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_k_ss10 = (batch_B * true_grad_norm_sq) / max(
                        k_ss_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_s_scv10 = (batch_B * true_grad_norm_sq) / max(
                        s_scv_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_s_svar10 = (batch_B * true_grad_norm_sq) / max(
                        s_svar_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_s_rcv10 = (batch_B * true_grad_norm_sq) / max(
                        s_rcv_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_s_rvar10 = (batch_B * true_grad_norm_sq) / max(
                        s_rvar_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_s_scv1 = (batch_B * true_grad_norm_sq) / max(
                        s_scv_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_s_svar1 = (batch_B * true_grad_norm_sq) / max(
                        s_svar_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_s_rcv1 = (batch_B * true_grad_norm_sq) / max(
                        s_rcv_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_s_rvar1 = (batch_B * true_grad_norm_sq) / max(
                        s_rvar_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_s_sqm1 = (batch_B * true_grad_norm_sq) / max(
                        s_ssqm_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_s_rsqm1 = (batch_B * true_grad_norm_sq) / max(
                        s_rsqm_sigma2_hat_mb1, 1e-12
                    )
                    alpha_crit_s_sqm10 = (batch_B * true_grad_norm_sq) / max(
                        s_ssqm_sigma2_hat_mb10, 1e-12
                    )
                    alpha_crit_s_rsqm10 = (batch_B * true_grad_norm_sq) / max(
                        s_rsqm_sigma2_hat_mb10, 1e-12
                    )
                    alphas = {
                    ...
                    s_svar_sum10 += max(
                        S_svar_pred_real10, union_eff_gt_acrit_step_svar10
                    )
                    ...
                    s_sqm_sum10 += max(
                        S_ssqm_pred_real10, union_eff_gt_acrit_step_sqm10
                    )
                    ...
                    if ly_sched is not None:
                        if config.param == "sqm10":
                            ...
                            lr_star, _ = ly_sched.step(
                                effective_lr, alpha_crit_t, total_updates, total_steps
                            )
                        elif config.param == "sqm1":
                            lr_star, _ = ly_sched.step(
                                effective_lr,
            ...
            if total_updates % config.log_interval == 0:
                ...
                log = {
                    ...
                    "weight_norm": wn,
                    "change_in_spectral_norm": spec_norm_change,
    ...
    if feature_diagnostics.should_run_feature_diagnostics(config, task):
        diag_info = feature_diagnostics.run_feature_diagnostics(
            model=model,
            dataset=train_dataset,
            device=device,
            cfg=config,
            task_idx=task,
            init_weights=feature_diag_init_weights,
            run_name=feature_diag_run_name,
        )
        if not diag_info.get("skipped", False):
            diag_log = {"feature_diag/task": task + 1}
            for layer_name, stats in diag_info["summary"].items():
                diag_log[f"feature_diag/{layer_name}/raw_corr"] = stats["raw_corr"]
                diag_log[f"feature_diag/{layer_name}/delta_corr"] = stats["delta_corr"]
                diag_log[f"feature_diag/{layer_name}/raw_diag_corr"] = stats[
                    "raw_diag_corr"
                ]
                diag_log[f"feature_diag/{layer_name}/delta_diag_corr"] = stats[
                    "delta_diag_corr"
                ]
            run.log(diag_log)
