...
parser.add_argument("--l2_lambda", type=float, default=0.0,
     help="lambda coefficient for L2 (weight-decay) regularization")
...
for i in range(10, 10 + args.runs):
    ...
    for _ in range(EPOCHS):
        for inputs, labels in train_loader:
            ...
            if args.reg == "l2_init":
                # TODO: implement L2‐from‐init: ∥W−W₀∥²
                pass
            elif args.reg == "wass":
                # TODO: implement Wasserstein‐based penalty
                pass
            elif args.reg == "spectral":
                # exact spectral‐norm penalty
                spec_reg = torch.tensor(0.0, device=device)
                for name, p in model.named_parameters():
                    if p.requires_grad and p.ndim >= 2:
                        sigma_1 = torch.linalg.svdvals(p)[0]
                        spec_reg = spec_reg + (sigma_1.pow(args.spectral_k) - 1.0).pow(2)
                reg_term = args.spectral_lambda * spec_reg
