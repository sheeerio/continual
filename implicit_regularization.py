...
parser.add_argument("--model", type=str, default="MLP", choices=[
    "MLP",
    "LayerNormMLP",
    "BatchNormMLP",
    "LeakyLayerNormMLP",
...
parser.add_argument("--dataset", type=str, default="MNIST", choices=["MNIST", "CIFAR10", "PermutedMNIST", "Shuffle_CIFAR", "Tiny_ImageNet"])
parser.add_argument("--activation", type=str, default="relu", choices=[
    "relu", "leaky_relu", "tanh", "identity", "crelu", "fourier", "adalin", "cleaky_relu", "softplus", "swish"
])
...
parser.add_argument("--reg", type=str, default="l2", choices=["l2", "l2_init", "wass", "spectral", "shrink_perturb"])
parser.add_argument("--wass_lambda", type=float, default=0.0)
parser.add_argument("--exp_name", type=str, default="")
parser.add_argument("--initialization", type=str, default="kaiming", choices=["kaiming", "xavier", "normal", "uniform"])
parser.add_argument(
    "--sp_weight_decay", type=float, default=0.0,
    help="Shrink factor (lambda) for shrink-and-perturb (weight decay per step)"
)
parser.add_argument(
    "--sp_noise_std", type=float, default=0.0,
    help="Standard deviation (gamma) of Gaussian noise for shrink-and-perturb"
)
parser.add_argument("--step_size_schedule", type=str, default="constant", choices=["constant", "linear", "exponential", "polynomial", "cosine"])
...
class MLP(nn.Module):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                if args.initialization == "kaiming":
                    nn.init.kaiming_uniform_(m.weight, a=args.alpha if args.activation == "adalin" else 0, nonlinearity=map[args.activation])
    ...
    def forward(self, x):
        if args.activation == "relu":
            ...
            x1 = self.fc1(x); x = torch.cat([F.relu(x1),F.relu(-x1)],1)
            x2 = self.fc2(x); x = torch.cat([F.relu(x2),F.relu(-x2)],1)
            x3 = self.fc3(x); x = torch.cat([F.relu(x3),F.relu(-x3)],1)
        elif args.activation == "cleaky_relu":
            x = self.fc1(x); x = torch.cat([F.leaky_relu(x), F.leaky_relu(-x)], 1)
            x = self.fc2(x); x = torch.cat([F.leaky_relu(x), F.leaky_relu(-x)], 1)
            x = self.fc3(x); x = torch.cat([F.leaky_relu(x), F.leaky_relu(-x)], 1)        
            ...
            x = torch.cat([torch.sin(self.fc1(x*5.0)),torch.cos(self.fc1(x*5.0))],1)
            x = torch.cat([torch.sin(self.fc2(x*5.0)),torch.cos(self.fc2(x*5.0))],1)
            x = torch.cat([torch.sin(self.fc3(x*5.0)),torch.cos(self.fc3(x*5.0))],1)
        elif args.activation == "softplus":
            x = F.softplus(self.fc1(x)); x = F.softplus(self.fc2(x)); x = F.softplus(self.fc3(x))
...
class LayerNormMLP(nn.Module):
    def __init__(self, i, h, o):
        super().__init__()
        self.fc1 = nn.Linear(i,h); self.fc2 = nn.Linear(h,h); self.fc3 = nn.Linear(h,h); self.fc4 = nn.Linear(h,o)
        self.ln1 = nn.LayerNorm(h); self.ln2 = nn.LayerNorm(h); self.ln3 = nn.LayerNorm(h)
    def forward(self, x):
        x = F.relu(self.ln1(self.fc1(x))); x = F.relu(self.ln2(self.fc2(x))); x = F.relu(self.ln3(self.fc3(x)))
        return self.fc4(x)

class BatchNormMLP(nn.Module):
    def __init__(self, i, h, o):
        super().__init__()
        self.fc1=nn.Linear(i,h); self.bn1=nn.BatchNorm1d(h); 
        if args.activation in ("crelu", "fourier", "cleaky_relu"):
            self.fc2=nn.Linear(2*h,h); self.fc3=nn.Linear(2*h,h); 
            self.fc4=nn.Linear(2*h,h); self.fc5=nn.Linear(2*h,o)
            self.bn2=nn.BatchNorm1d(2*h); self.bn3=nn.BatchNorm1d(2*h)
            self.bn4=nn.BatchNorm1d(h)
        else:
            self.fc2=nn.Linear(h,h); self.fc3=nn.Linear(h,h); 
            self.fc4=nn.Linear(h,h); self.fc5=nn.Linear(h,o)
        self.bn2=nn.BatchNorm1d(h); self.bn3=nn.BatchNorm1d(h); 
        self.bn4=nn.BatchNorm1d(h)
        for m in self.modules():
            if isinstance(m, nn.Linear):
                if args.initialization == "kaiming":
    ...
    def forward(self, x):
        if args.activation == "relu":
            x = F.relu(self.bn1(self.fc1(x))); x = F.relu(self.bn2(self.fc2(x))); x = F.relu(self.bn3(self.fc3(x)))
            x = F.relu(self.bn4(self.fc4(x)))
        elif args.activation == "leaky_relu":
            x = F.leaky_relu(self.bn1(self.fc1(x))); x = F.leaky_relu(self.bn2(self.fc2(x))); x = F.leaky_relu(self.bn3(self.fc3(x)))
            ...
            x1 = self.fc1(x); x = torch.cat([F.relu(self.bn1(x1)), F.relu(-x1)], 1)
            x2 = self.fc2(x); x = torch.cat([F.relu(self.bn2(x2)), F.relu(-x2)], 1)
            x3 = self.fc3(x); x = torch.cat([F.relu(self.bn3(x3)), F.relu(-x3)], 1)
            x4 = self.fc4(x); x = torch.cat([F.relu(self.bn4(x4)), F.relu(-x4)], 1)
        elif args.activation == "softplus":
            x = F.softplus(self.bn1(self.fc1(x))); x = F.softplus(self.bn2(self.fc2(x))); x = F.softplus(self.bn3(self.fc3(x)))
...
class LeakyLayerNormMLP(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(self.ln1(self.fc1(x))); x = F.leaky_relu(self.ln2(self.fc2(x)))
        return self.fc3(x)
...
class CNN(nn.Module):
    def __init__(self, in_channels):
        ...
        self.fc1=nn.Linear(128*2*2,512); self.fc2=nn.Linear(512,10)
    def forward(self, x):
        x=F.relu(self.conv1(x)); x=F.relu(self.conv2(x))
        x=F.relu(self.conv3(x)); x=F.relu(self.conv4(x))
        x=self.flatten(x); x=F.relu(self.fc1(x))
        return self.fc2(x)

class BatchNormCNN(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.conv1=nn.Conv2d(in_channels,16,3,stride=2,padding=1); self.bn1=nn.BatchNorm2d(16)
        self.conv2=nn.Conv2d(16,32,3,stride=2,padding=1); self.bn2=nn.BatchNorm2d(32)
        self.conv3=nn.Conv2d(32,64,3,stride=2,padding=1); self.bn3=nn.BatchNorm2d(64)
        self.conv4=nn.Conv2d(64,128,3,stride=2,padding=1); self.bn4=nn.BatchNorm2d(128)
        self.flatten=nn.Flatten()
        self.fc1=nn.Linear(128*2*2,512); self.bn5=nn.BatchNorm1d(512); self.fc2=nn.Linear(512,10)
    def forward(self, x):
        ...
        x=self.flatten(x); x=F.relu(self.bn5(self.fc1(x)))
        return self.fc2(x)
...
if args.dataset == "MNIST":
    ...
    tf = transforms.Compose([transforms.ToTensor(), transforms.Normalize(DATA_MEAN, DATA_STD)])
    ...
    DATA_MEAN = (tmp.data / 255.0).mean(axis=(0,1,2))
    DATA_STD  = (tmp.data / 255.0).std(axis=(0,1,2))
    tf = transforms.Compose([transforms.ToTensor(), transforms.Normalize(tuple(DATA_MEAN.tolist()), tuple(DATA_STD.tolist()))])
...
if args.model in ["MLP","LayerNormMLP","BatchNormMLP","LeakyLayerNormMLP","LeakyKaimingLayerNormMLP"]:
...
run = wandb.init(
    ...
    config={
        ...
        "activation":args.activation, "reg":args.reg,
        "batch_size":args.batch_size, "runs":args.runs,
        "random_seed":args.seed, "l2_lambda":args.l2_lambda,
        "spectral_lambda":args.spectral_lambda, "spectral_k":args.spectral_k,
...
for task in range(10,10+args.runs):
    ...
    if args.dataset == "PermutedMNIST":
        ...
        loader = data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4)
    elif args.dataset == "Shuffle_CIFAR":
    ...
    for _ in range(args.epochs):
        ...
        for x,y in loader:
            ...
            if total_updates % args.log_interval==0:
                ...
                lam_std  = (sum((x - lam_mean)**2 for x in sharp_queue) / len(sharp_queue))**0.5
            ...
            delta = torch.cat([(p.data - o).view(-1).abs() for p,o in zip(params,old)])
            ...
            if total_updates % args.log_interval==0:
                ...
                "hessian_rank": hessian_rank if total_updates % args.log_interval==0 else None,
                "trace_val": trace_val if total_updates % TRACE_INTERVAL==0 else None,
                "sharpness": sharpness,
    ...
    model.eval()
    eval_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False)
    total, count = 0.0, 0
    with torch.no_grad():
        for x,y in eval_loader:
            ...
            bs = y.size(0)
            total += l.item()*bs; count += bs
    J = total/count
    pn = torch.cat([p.data.view(-1).abs() for p in model.parameters() if p.requires_grad]).mean().item()
    ...
    run.log({"J":J, "param_norm":pn, "avg_norm_sharp": this_normalized_sharp / (args.epochs * len(loader)), "average_update_norm":aun, "effective_rank":effective_rank, "task_acc": this_task_acc / (args.epochs * len(loader))})
...
for m in ["batch_error","param_norm","update_norm"]:
    ...
    plt.xlabel("Task"); plt.ylabel(m); plt.legend(); plt.show()