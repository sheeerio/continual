...
def register_activation_hooks(
    model: nn.Module, model_name: str, store: Dict[str, torch.Tensor]
) -> None:
...
def build_optimizer(config, layer_groups, weight_decay: float):
    ...
    if config.optimizer == "sgd":
        return optim.SGD(
            layer_groups, lr=config.lr, weight_decay=weight_decay, momentum=0.9
        )
    if config.optimizer == "clamped_adam":
        return optimizers.ClampedAdam(
...
def build_scheduler(
    config, optimizer, sharp_state, layer_states, act_layer_states, layer_map
):
    total_steps = None
    if config.lr_schedule in {
        "linear",
        "step",
        "exponential",
        "polynomial",
...
class L2InitReg(Regularizer):
    def __init__(self, config, init_params):
        self.cfg, self.init = config, init_params
    ...
    def __call__(self, model: nn.Module, device) -> torch.Tensor:
        ...
        for _n, p in model.named_parameters():
            if p.requires_grad and p.ndim >= 2:
                reg += (
                    optimizers.power_iteration(p, 1).pow(self.cfg.spectral_k) - 1.0
                ).pow(2)
    ...
    def __call__(self, model: nn.Module, device) -> torch.Tensor:
        ...
        for name, p in model.named_parameters():
            ...
            if self.total_updates_ref() % self.cfg.ortho_interval == 0:
                ...
                sigma_now = self.cached.get(
                    name, optimizers.power_iteration_sigma_min(p, iters=1).detach()
                )
...
def make_regularizer(
    config, init_params, init_sigma_min, cached_sigma_min, total_updates_ref
):
    if config.reg == "l2_init":
        return L2InitReg(config, init_params)
    if config.reg == "wass":
        return WassersteinReg(config, init_params)
    if config.reg == "spectral":
        return SpectralReg(config)
    if config.reg == "ortho":
        return OrthoSigmaMinReg(
            config, init_sigma_min, cached_sigma_min, total_updates_ref
        )
    if config.reg == "orthofrob":
        return OrthoFrobReg(config)
    return Regularizer()
...
class TrainingState:
    ...
    sharp_state: misc.EMAState = field(
        default_factory=lambda: misc.EMAState(alphas=(0.01, 0.05, 0.5))
    )
    r_sharp_state: misc.EMAState = field(
        default_factory=lambda: misc.EMAState(alphas=(0.01, 0.05, 0.5))
    )
...
class Trainer:
    def __init__(self, config):
        ...
        self.layer_states = {
            layer: misc.EMAState(alphas=(0.01, 0.05, 0.5)) for layer in self.layer_map
        }
        self.act_layer_states = {
            layer: misc.EMAState(alphas=(0.01, 0.05, 0.5)) for layer in self.layer_map
        }
        ...
        with torch.no_grad():
            for name, p in self.model.named_parameters():
                if p.requires_grad and p.ndim >= 2:
                    self.init_sigma_min[name] = optimizers.power_iteration_sigma_min(
                        p.detach(), iters=3
                    ).item()
        self.cached_sigma_min = {}

        total_updates_ref = lambda: self.state.total_updates
        self.regularizer = make_regularizer(
            config,
            self.init_params,
            self.init_sigma_min,
            self.cached_sigma_min,
    ...
    def _reset_task_accumulators(self):
        for n in [
            ...
            "ly_snr_2_sum",
            "ly_union_sum",
            "eff_acrit_scv1_union_sum",
            "eff_acrit_ss1_union_sum",
            "eff_acrit_svar1_union_sum",
            "eff_acrit_ssqm1_union_sum",
            "eff_acrit_scv10_union_sum",
            "eff_acrit_ss10_union_sum",
            "eff_acrit_svar10_union_sum",
            "eff_acrit_ssqm10_union_sum",
            "eff_acrit_rcv1_union_sum",
            "eff_acrit_rs1_union_sum",
            "eff_acrit_rvar1_union_sum",
            "eff_acrit_rsqm1_union_sum",
            "eff_acrit_rcv10_union_sum",
            "eff_acrit_rs10_union_sum",
            "eff_acrit_rvar10_union_sum",
            "eff_acrit_rsqm10_union_sum",
        ]:
            setattr(self, n, 0.0)
    ...
    def _build_loader_for_task(self, task_idx: int):
        ...
        loader = data.DataLoader(
            train_ds, batch_size=self.cfg.batch_size, shuffle=True, num_workers=4
        )
        return train_ds, loader

    def _metrics_schedules_and_logging(
        self, inputs, labels, loss, epoch: int, total_steps: int
    ):
        def safe_div(a, b):
            return a / (b + 1e-12)
        ...
        for n in [
            ...
            "eff_acrit_rsqm10_union_sum",
            "this_task_acc",
            "this_normalized_sharp",
        ]:
            init_acc(n)

        if self.cfg.optimizer == "adam":
            layer_eff_lrs = optimizers.per_layer_effective_lr(
        ...
        for layer, params in self.layer_map.items():
            ...
            if len(gi) == 0:
                ...
                g2 = float(
                    torch.cat([g.contiguous().view(-1) for g in gi]).pow(2).sum().item()
                )
            ...
            over("eff_gt_scv1", "s_scv", 1)
            over("eff_gt_ss1", "s_ss", 1)
            over("eff_gt_svar1", "s_svar", 1)
            over("eff_gt_sqm1", "s_sqm", 1)
            over("eff_gt_scv10", "s_scv", 10)
            over("eff_gt_ss10", "s_ss", 10)
            over("eff_gt_svar10", "s_svar", 10)
            over("eff_gt_sqm10", "s_sqm", 10)
            over("eff_gt_rcv1", "r_rcv", 1)
            over("eff_gt_rs1", "r_rs", 1)
            over("eff_gt_rvar1", "r_rvar", 1)
            over("eff_gt_rsqm1", "r_rsqm", 1)
            over("eff_gt_rcv10", "r_rcv", 10)
            over("eff_gt_rs10", "r_rs", 10)
            over("eff_gt_rvar10", "r_rvar", 10)
            over("eff_gt_rsqm10", "r_rsqm", 10)
            ...
            if self.pl_sched is not None:
                ...
                if self.cfg.param in ("s_tau", "r_tau"):
                    ...
                    target = alpha[(t, m)]
                self.pl_sched.step(
                    layer, eff_lr, target, self.state.total_updates, total_steps
                )
        ...
        eff_lr = optimizers.compute_effective_lr(
            self.optimizer, cfg=self.cfg, step=self.state.total_updates, sgd_mode="time"
        )

        self.sharp_state, sharp_stats = misc.update_stat(
            norm_sharp, self.sharp_state, eff_lr
        )
        self.r_sharp_state, rsharp_stats = misc.update_stat(
            sharp, self.r_sharp_state, eff_lr
        )
        ...
        sigma2_hat_mb = optimizers.grad_variance_within_batch(
            self.model, self.criterion_nored, inputs, labels
        )
        B = int(inputs.size(0))
        ...
        def mixK(coef, val):
            return sigma2_hat_mb + coef * g2_true * val

        mixes_global = {
            "K_rs_1": mixK(10.0, sharp),
            "K_ss_1": mixK(10.0, norm_sharp),
            "K_rs_10": mixK(20.0, sharp),
            "K_ss_10": mixK(20.0, norm_sharp),
            "S_scv_10": mixK(20.0, sharp_stats["lam_cv"]),
            "S_svar_10": mixK(20.0, sharp_stats["lam_var"]),
            "S_rcv_10": mixK(20.0, rsharp_stats["lam_cv"]),
            "S_rvar_10": mixK(20.0, rsharp_stats["lam_var"]),
            "S_scv_1": mixK(10.0, sharp_stats["lam_cv"]),
            "S_svar_1": mixK(10.0, sharp_stats["lam_var"]),
            "S_rcv_1": mixK(10.0, rsharp_stats["lam_cv"]),
            "S_rvar_1": mixK(10.0, rsharp_stats["lam_var"]),
            "S_ssqm_10": mixK(20.0, sharp_stats["sq_mean"]),
            "S_rsqm_10": mixK(20.0, rsharp_stats["sq_mean"]),
            "S_ssqm_1": mixK(75.0, sharp_stats["sq_mean"]),
            "S_rsqm_1": mixK(10.0, rsharp_stats["sq_mean"]),
        }
        ratios = {
            k: eff_lr * (v / max(1, B)) / (g2_true + 1e-12)
            for k, v in mixes_global.items()
        }

        def upd(x):
            return self.state.snr_predictor.update(x, T_t)[1]
        ...
        alpha = {
            ...
            "k_rs10": getA("K_rs_10"),
            "k_ss10": getA("K_ss_10"),
            "s_scv10": getA("S_scv_10"),
            "s_svar10": getA("S_svar_10"),
            "s_rcv10": getA("S_rcv_10"),
            "s_rvar10": getA("S_rvar_10"),
            "s_scv1": getA("S_scv_1"),
            "s_svar1": getA("S_svar_1"),
            "s_rcv1": getA("S_rcv_1"),
            "s_rvar1": getA("S_rvar_1"),
            "s_sqm1": getA("S_ssqm_1"),
            "s_rsqm1": getA("S_rsqm_1"),
            "s_sqm10": getA("S_ssqm_10"),
            "s_rsqm10": getA("S_rsqm_10"),
        }
        ...
        if self.ly_sched is not None:
            ...
            lr_star, _ = self.ly_sched.step(
                eff_lr, target, self.state.total_updates, total_steps
            )
        ...
        if c.lr_schedule == "linear":
            ...
            self.pt_scheduler = optim.lr_scheduler.LambdaLR(
                ...
                lr_lambda=lambda step: max(
                    0.0, (initial_lr - (decay_range * step / total_steps)) / initial_lr
                ),
            )
        elif c.lr_schedule == "step":
            self.pt_scheduler = optim.lr_scheduler.StepLR(
                self.optimizer, step_size=c.step_size, gamma=c.gamma
            )
        elif c.lr_schedule == "exponential":
            self.pt_scheduler = optim.lr_scheduler.ExponentialLR(
                self.optimizer, gamma=c.gamma
            )
        elif c.lr_schedule == "polynomial":
            self.pt_scheduler = optim.lr_scheduler.LambdaLR(
                self.optimizer,
                lr_lambda=lambda step: (1 - step / total_steps) ** c.power,
            )
        elif c.lr_schedule == "cosine":
            self.pt_scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
                self.optimizer, T_0=c.epochs
            )
        elif c.lr_schedule == "wsd":
            self.pt_scheduler = optim.lr_scheduler.LambdaLR(
                self.optimizer, lr_lambda=schedulers.wsd_lambda
            )
        elif c.lr_schedule == "power":
            self.pt_scheduler = optim.lr_scheduler.LambdaLR(
                self.optimizer, lr_lambda=schedulers.power_lambda
            )
        elif c.lr_schedule == "skew":
            self.pt_scheduler = optim.lr_scheduler.LambdaLR(
                self.optimizer, lr_lambda=schedulers.skew_lambda
            )

    def train(self):
        LENGTH_CHOICES = [100, 300, 50, 150]
        task_lengths = (
            [random.choice(LENGTH_CHOICES) for _ in range(self.cfg.runs)]
            if self.cfg.random_length
            else [self.cfg.epochs] * self.cfg.runs
        )
        ...
        for task in range(self.cfg.runs):
                ...
                for x, y in loader:
                    ...
                    if self.cfg.sam:
                        ...
                        for p, e in zip(params, epsilons):
                            p.data.add_(e)
                        out_adv = self.model(inputs)
                        loss_adv = self.criterion(out_adv, labels) + reg
                        self.optimizer.zero_grad()
                        loss_adv.backward()
                        for p, e in zip(params, epsilons):
                            p.data.sub_(e)
                    ...
                    if self.state.total_updates % self.cfg.log_interval == 0:
                        self._metrics_schedules_and_logging(
                            inputs, labels, loss, epoch, total_steps
                        )

                    self.optimizer.step()

                    if self.cfg.optimizer == "adam":
                        self.optimizer.param_groups[0]["betas"] = optimizers.get_betas(
                            self.cfg, epoch
                        )
                    ...
                    with torch.no_grad():
                        params = [p for p in self.model.parameters() if p.requires_grad]
                        wn = (
                            torch.cat([p.data.view(-1).abs() for p in params])
                            .mean()
                            .item()
                        )
                        update_norm = (
                            torch.cat([p.grad.view(-1).abs() for p in params])
                            .mean()
                            .item()
            ...
            with torch.no_grad():
                for x, y in eval_loader:
                    inp = (
                        x.to(self.device)
                        if self.cfg.model in ["CNN", "BatchNormCNN"]
                        else x.view(x.size(0), -1).to(self.device)
                    )
                    out = self.model(inp)
                    l = self.criterion(out, y.to(self.device))
                    bs = y.size(0)
                    total += l.item() * bs
                    count += bs
            J = total / count
            ...
            inputs = (
                inputs.view(inputs.size(0), -1).to(self.device)
                if self.cfg.model not in ["CNN", "BatchNormCNN"]
            ...
            steps = (
                (epochs * len(loader) / self.cfg.log_interval)
                if self.cfg.log_interval
                else 1
            )
            wandb.log(
                {
                    ...
                    "eff_acrit_scv1_pct_union": 1
                    - (self.eff_acrit_scv1_union_sum / steps),
                    "eff_acrit_ss1_pct_union": 1
                    - (self.eff_acrit_ss1_union_sum / steps),
                    "eff_acrit_svar1_pct_union": 1
                    - (self.eff_acrit_svar1_union_sum / steps),
                    "eff_acrit_scv10_pct_union": 1
                    - (self.eff_acrit_scv10_union_sum / steps),
                    "eff_acrit_ss10_pct_union": 1
                    - (self.eff_acrit_ss10_union_sum / steps),
                    "eff_acrit_svar10_pct_union": 1
                    - (self.eff_acrit_svar10_union_sum / steps),
                    "eff_acrit_ssqm1_pct_union": 1
                    - (self.eff_acrit_ssqm1_union_sum / steps),
                    "eff_acrit_ssqm10_pct_union": 1
                    - (self.eff_acrit_ssqm10_union_sum / steps),
                    "eff_acrit_rcv1_pct_union": 1
                    - (self.eff_acrit_rcv1_union_sum / steps),
                    "eff_acrit_rs1_pct_union": 1
                    - (self.eff_acrit_rs1_union_sum / steps),
                    "eff_acrit_rvar1_pct_union": 1
                    - (self.eff_acrit_rvar1_union_sum / steps),
                    "eff_acrit_rsqm1_pct_union": 1
                    - (self.eff_acrit_rsqm1_union_sum / steps),
                    "eff_acrit_rcv10_pct_union": 1
                    - (self.eff_acrit_rcv10_union_sum / steps),
                    "eff_acrit_rs10_pct_union": 1
                    - (self.eff_acrit_rs10_union_sum / steps),
                    "eff_acrit_rvar10_pct_union": 1
                    - (self.eff_acrit_rvar10_union_sum / steps),
                    "eff_acrit_rsqm10_pct_union": 1
                    - (self.eff_acrit_rsqm10_union_sum / steps),
                }
            )
