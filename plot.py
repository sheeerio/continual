...
history_raw = target_run.history(
    keys=[metric_task, metric_snr1, metric_snr2, metric_snr3]
)
...
history_df = history_df.dropna(
    subset=[metric_task, metric_snr1, metric_snr2, metric_snr3]
)
...
if config.type == "normalize":
    ...
    ly_snr_pct2_scaled = (ly_snr_pct2 - ly_snr_pct2.min()) / (
        ly_snr_pct2.max() - ly_snr_pct2.min()
    )
...
plt.plot(
    snr_pct_scaled, label=f"{metric_snr1} (scaled)", linestyle="--", color="orange"
)
plt.plot(
    ly_snr_pct2_scaled, label=f"{metric_snr2} (scaled)", linestyle="--", color="green"
)
plt.plot(k_snr_pct_scaled, label=f"{metric_snr3} (scaled)", linestyle="--", color="red")

if config.type == "normalize":
    plt.title(
        f"{metric_task}, {metric_snr1}, {metric_snr2}, {metric_snr3} (all scaled)\nRun '{run_name}'"
    )
else:
    plt.title(
        f"{metric_task}, {metric_snr1} (scaled), {metric_snr2} (scaled), {metric_snr3} (scaled)\nRun '{run_name}'"
    )
...
# Save + show
plt.savefig(
    f"plots/{config.type}/{run_name}_{group_name}_metrics_scaled_range.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
