...
Example:
  python plot_ratio.py \
    ...
    --name  "identity" \
    --name2 "adalin3" \
    --label1 "identity" --label2 "leaky_relu" --color2 tab:orange \
    --acc-key task_acc \
    --rank-key ratio \
    --rank-stat last \
    --smooth-seeds 0.1 \
...
def main():
    ...
    axR.set_ylabel("Ratio (dashed)", fontsize=20)
    title = f""
    axL.set_title(title)
    ...
    axL.legend(hL + hR, lL + lR, loc="upper right", fontsize=15)
