...
Example:
  python plot_sharpness.py \
    ...
    --band minmax \
    --min-seeds 2 \
    --rank-yscale log --rank-ymin 800 \
    --out plots/sharpness_false.pdf
"""
...
def parse_args():
    ...
    p.add_argument("--rank-key", default="hessian_rank")   # or effective_rank / sharpness
    ...
    # right-axis scaling (sharpness / rank)
    p.add_argument("--rank-yscale", choices=["linear","log"], default="linear",
                   help="Y scale for the right axis (rank/sharpness).")
    p.add_argument("--rank-ymin", type=float, default=None,
                   help="Lower bound for right y-axis (e.g., 800).")
    p.add_argument("--rank-ymax", type=float, default=None,
                   help="Upper bound for right y-axis (optional).")
...
def main():
    ...
    plot_cohort(cohort1, args.color1, r"$\alpha$-lin7")
    ...
    if args.rank_yscale == "log":
        axR.set_yscale("log")
    if args.rank_ymin is not None or args.rank_ymax is not None:
        axR.set_ylim(bottom=args.rank_ymin, top=args.rank_ymax)

    right_label = "Sharpness (dashed)"
    if args.rank_yscale == "log":
        right_label = "Sharpness [log] (dashed)"
    axR.set_ylabel(right_label, fontsize=20)
